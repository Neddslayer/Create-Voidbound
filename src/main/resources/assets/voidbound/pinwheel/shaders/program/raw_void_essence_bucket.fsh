...
void main() {
    ...
    vec2 uv = vec2(texCoord0.x * 512 * (atlasSize.x / atlasSize.y), texCoord0.y * 512);

    float random = rand(pixelate((uv + rand(vec2(VeilRenderTime)) * 50), 16));

    float cn = clamp(2 * cnoise( pixelate(vec3(uv.x, 0, uv.y), 2) + vec3(0, VeilRenderTime, 0) ), -1, 1);
    vec3 noise = (pow(cn, 2) - pow(cn, 3)) * vec3(0.8, 0, 1) * random;
    vec4 originalColor = texture(BlockAtlas, texCoord0) * ColorModulator;
    vec4 color = originalColor * vertexColor; // applies lighting
